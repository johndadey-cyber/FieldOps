...
final class AvailabilityOverride
{
    ...
    {
        $sql = "SELECT id, employee_id, date, status, type, start_time, end_time, reason
    ...
    public static function save(PDO $pdo, array $data): int
    {
        ...
        $sql = $id > 0
              ? "UPDATE employee_availability_overrides
                   SET employee_id=:eid, date=:d, status=:s, type=:t, start_time=:st, end_time=:et, reason=:r
                   WHERE id=:id"
              : "INSERT INTO employee_availability_overrides (employee_id, date, status, type, start_time, end_time, reason)
                   VALUES (:eid,:d,:s,:t,:st,:et,:r)";
